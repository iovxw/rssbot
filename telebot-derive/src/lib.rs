...
fn expand_setter(ast: syn::MacroInput) -> quote::Tokens {
    ...
    let field_optional: Vec<_> = fields.iter()
        .filter(|f| is_option_ident(&f))
        .map(|f| f.0)
        .collect();
    let field_optional2 = field_optional.clone();

    let field_compulsory2: Vec<_> = fields.iter()
        .map(|f| f.0)
...
fn expand_function(ast: syn::MacroInput) -> quote::Tokens {
    ...
    let ty_compulsory_generic: Vec<_> =
        (0..ty_compulsory.len()).map(|t| syn::Ident::from(format!("T{}", t))).collect();
                ...
                pub fn send<'a>(self)
                    ...
                    result::<#wrapper_name, (#wrapper_name, Error)>(Ok(self))
                        ...
                        .and_then(|mut tmp| {
                            ...
                            tmp.bot.fetch_formdata(#function,
                                                   msg,
                                                   file.source,
                                                   #bot_function_name,
                                                   &file.name)
                        ...
                        .map(move |answer| (RcBot { inner: cloned_bot },
