...
pub fn spawn_subscriber_alive_checker(bot: telebot::RcBot, db: data::Database, handle: Handle) {
    handle.clone().spawn(Interval::new(Duration::from_secs(12 * 60 * 60), &handle)
                        ...
                        match chat.kind.as_str() {
                            ...
                            .or_else(move |_| {
                                bot.get_chat_member(subscriber, bot.inner.id)
                                    .send()
                                    .map(move |(_, chat_member)| match chat_member.status
                                                   .as_str() {
                                             "left" | "kicked" => db2.delete_subscriber(subscriber),
